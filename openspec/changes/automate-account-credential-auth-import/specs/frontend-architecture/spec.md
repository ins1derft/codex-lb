...
#### Scenario: Account import (auth.json)
...
- **WHEN** a user opens the import dialog in credentials mode and submits multi-line data in `email:password:2fa_secret` format
- **THEN** the app calls `POST /api/accounts/import-credentials`
- **AND** shows imported/failed counts with per-line results
- **AND** refreshes account list after completion
