...
class DashboardService:
    ...
    async def get_overview(
        ...
        *,
        ...
        top_error = await self._repo.top_error_since(bucket_since, owner_user_id=owner_user_id)
        ...
        activity_aggregate = await self._repo.aggregate_activity_since(bucket_since, owner_user_id=owner_user_id)
        ...
        for account_id in all_account_ids:
            if account_id in normalized_primary_ids:
                ...
                if account_id in secondary_usage:
                    ...
                    sec_acct_since = now - timedelta(minutes=sec_window)
                    sec_fetch_ids.append(account_id)
                    sec_cutoffs[account_id] = sec_acct_since
                    if sec_acct_since < sec_since:
                        sec_since = sec_acct_since
        ...
        for account_id in all_account_ids:
            if account_id in normalized_primary_ids:
                ...
                if account_id in sec_cutoffs:
                    sec_cutoff = sec_cutoffs[account_id]
                    sec_rows = [row for row in all_sec_rows.get(account_id, []) if row.recorded_at >= sec_cutoff]
                    if sec_rows:
...
def _build_depletion_by_window(
    ...
    def _aggregate(history: dict[str, list[UsageHistory]], window: str) -> DepletionResponse | None:
        ...
        for account_id, rows in history.items():
            metric = compute_depletion_for_account(
